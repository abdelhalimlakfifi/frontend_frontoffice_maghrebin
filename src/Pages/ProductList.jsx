...
import { ProductServices } from "./data/productServices";
...
function ProductList() {
    ...
    function formatNumberAsFloat(inputNumber) {
                        ...
                        {filteredProducts ? (
                            <section className="grid place-items-center gap-7 grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                {filteredProducts.map((card, index) => {
                                    ...
                                    console.log(card.title)
                                    const mainImg = card.images.find((c) => {
                                        if(c.image_id !== null){
                                            return c.image_id.main == true
                                        }
                                    });
                                    const secondaryImg = card.images.find((c) => {
                                        if(c.image_id !== null){
                                            return c.image_id.secondary == true
                                        }
                                    });
                                    return (
                                        <Card key={index}
                                            id={card._id}
                                            title={card.title}
                                            price={`${formatNumberAsFloat(card.price)} MAD`}
                                            mainImg={`http://localhost:3000/api/${mainImg.image_id.path}`}
                                            secondaryImg={`http://localhost:3000/api/${secondaryImg.image_id.path}`}
                                        />
                                    )})}
                            </section>
                        ) : (
